...
inline void Solver::reset_iterators(double where) {
	if(verbose_now()) cout << "reset_iterators - where = "<< where << endl;
	...
	if (verbose_now()) cout << "m_Score2Vars_it->first = " << m_Score2Vars_it->first <<endl;
	...
	while (qhead < trail.size()) { 
		...
		for (vector<int>::reverse_iterator it = watches[NegatedLit].rbegin(); it != watches[NegatedLit].rend() && conflicting_clause_idx < 0; ++it) {
			...
			case ClauseState::C_UNIT: { // new implication				
				...
				if (verbose_now()) cout <<"antecedent["<<l2v(other_watch)<<"] = " <<*it <<endl;
				// ours start
				if (verbose_now()) cout<<"other_watch literal is" <<l2rl(other_watch )<<endl;
...
int Solver::analyze(const Clause conflicting) {
	...
	Clause	current_clause = conflicting, 
			new_clause;
	//print_cnf_state();
	if (verbose_now()) print_antecedents();
	int resolve_num = 0,
		bktrk = 0, 
...
void Solver::updateAntecedentForVariable(int clause_index, int recalculated_index) {
...
vector<int>  Solver::deletion_candidates_creation_and_updating_IndexRecalculationMap(map <int, int>& index_recalculation_map) {
	...
	* we need to change all indices of not deleted clauses
	*/
...
void Solver::update_maps_watchers_antecedents(map <int, int>& index_recalculation_map) {
	...
	for (int clause_index = 0; clause_index < cnf.size(); clause_index++) {
		...
		updateAntecedentForVariable(clause_index, recalculated_index);
	}
}
...
vector<int> Solver::cnf_update(vector <int>& clauses_to_be_deleted) {
	...
	for (int i = 0; i < clauses_to_be_deleted.size(); i++) {
		...
		cnf.erase(cnf.begin() + (clauses_to_be_deleted[i] - i)); 
	}
	return clauses_to_be_deleted;
}

void Solver::dynamic_backtrack(int k) {
	if (verbose_now()) cout << "dynamic_backtrack()" << endl;
	...
	while (true) {
		...
		while (true) {
            ...
            if ( num_curr_dr_conflicts > 20000 + 500 * num_deletion && updatedActivityScores_map.empty()) {	// "dynamic restart"
                ...
                num_deletion++;
				if (verbose_now()) {
					cout << "dynamic backtracking to level: " << dr_bktrc << endl;
